...
namespace CCodeGen.Modules
{
	...
	public abstract class CCodeMethodModule : CCodeStructModule
	{
		...
		public virtual void generate_method_result_declaration(Method m, CCodeFile decl_space, CCodeFunction cfunc, Dictionary<int, CCodeParameter> cparam_map, Dictionary<int, CCodeExpression> carg_map) {
			...
			if (m.return_type.is_real_non_null_struct_type()) {
				...
				if (carg_map != null) {
					carg_map[get_param_pos(-3)] = get_result_cexpression();
				}
				...
				for (int dim = 1; dim <= array_type.rank; dim++) {
					...
					cparam_map[get_param_pos(get_ccode_array_length_pos(m) + 0.01 * dim)] = cparam;
					if (carg_map != null) {
						carg_map[get_param_pos(get_ccode_array_length_pos(m) + 0.01 * dim)] = get_variable_cexpression(cparam.name);
					}
				}
				...
				if (d.has_target) {
					...
					cparam_map[get_param_pos(get_ccode_delegate_target_pos(m))] = cparam;
					if (carg_map != null) {
						carg_map[get_param_pos(get_ccode_delegate_target_pos(m))] = get_variable_cexpression(cparam.name);
					}
					if (deleg_type.is_disposable()) {
						cparam = new CCodeParameter(get_delegate_target_destroy_notify_cname("result"), "GDestroyNotify*");
						cparam_map[get_param_pos(get_ccode_delegate_target_pos(m) + 0.01)] = cparam;
						if (carg_map != null) {
							carg_map[get_param_pos(get_ccode_delegate_target_pos(m) + 0.01)] = get_variable_cexpression(cparam.name);
						}
					}
				}
			}

			if (m.get_error_types().Count > 0 || (m.base_method != null && m.base_method.get_error_types().Count > 0) || (m.base_interface_method != null && m.base_interface_method.get_error_types().Count > 0)) {
				...
				if (carg_map != null) {
					carg_map[get_param_pos(-1)] = new CCodeIdentifier(cparam.name);
				}
			}
		}
		...
		public void complete_async() {
			...
			var async_result_expr = CCodeMemberAccess.pointer(data_var, "_async_result");
			...
			var state = CCodeMemberAccess.pointer(data_var, "_state_");
			...
			if (context.require_glib_version(2, 44)) {
				...
				var task_complete = CCodeMemberAccess.pointer(data_var, "_task_complete_");
				task_is_complete = new CCodeBinaryExpression(CCodeBinaryOperator.INEQUALITY, task_complete, new CCodeConstant("TRUE"));
			}
		...
		void register_plugin_types(Symbol sym, HashSet<Symbol> registered_types) {
		...
		void register_plugin_type(ObjectTypeSymbol type_symbol, HashSet<Symbol> registered_types) {
		...
		public override void visit_method(Method m) {
			...
			CCodeBlock co_switch_block = null;
			...
			if (!m.is_abstract || (m.is_abstract && current_type_symbol is Class)) {
				if (m.body != null) {
					if (m.coroutine) {
						ccode.open_switch(CCodeMemberAccess.pointer(new CCodeIdentifier("_data_"), "_state_"));
					...
					if (m.closure) {
						...
						while (true) {
							...
							var parent_data = CCodeMemberAccess.pointer(new CCodeIdentifier("_data%d_".printf(block_id)), "_data%d_".printf(parent_block_id));
						...
						if (m.binding == MemberBinding.INSTANCE) {
							var cself = CCodeMemberAccess.pointer(new CCodeIdentifier("_data%d_".printf(block_id)), "self");
						...
						foreach (var type_param in m.get_type_parameters()) {
							...
							ccode.add_assignment(new CCodeIdentifier(func_name), CCodeMemberAccess.pointer(get_variable_cexpression("_data%d_".printf(block_id)), func_name));
					...
					foreach (Parameter param in m.get_parameters()) {
						...
						if (param.direction != ParameterDirection.OUT) {
							...
							var vardecl = CCodeVariableDeclarator.zero(get_variable_cname("_vala_%s".printf(param.name)), default_value_for_type(param.variable_type, true));
							...
							if (param.variable_type is ArrayType) {
								...
								if (!array_type.fixed_length) {
									for (int dim = 1; dim <= array_type.rank; dim++) {
										vardecl = CCodeVariableDeclarator.zero(get_array_length_cname(get_variable_cname("_vala_%s".printf(param.name)), dim), new CCodeConstant("0"));
								...
								if (d.has_target) {
									...
									vardecl = CCodeVariableDeclarator.zero("_vala_%s".printf(get_ccode_delegate_target_name(param)), new CCodeConstant("NULL"));
									ccode.add_declaration("void *", vardecl);

									if (deleg_type.is_disposable()) {
										vardecl = CCodeVariableDeclarator.zero(get_delegate_target_destroy_notify_cname(get_variable_cname("_vala_%s".printf(param.name))), new CCodeConstant("NULL"));
					...
					if (m is CreationMethod) {
						if (in_gobject_creation_method) {
							if (!m.coroutine) {
								ccode.add_declaration("%s *".printf(get_ccode_name(current_type_symbol)), CCodeVariableDeclarator.zero("self", new CCodeConstant("NULL")));
							}
						} else if (is_gtypeinstance_creation_method(m)) {
							var cl = (Class)m.parent_symbol;
							ccode.add_declaration(get_ccode_name(cl) + "*", CCodeVariableDeclarator.zero("self", new CCodeConstant("NULL")));
							...
							if (cl.is_fundamental() && !((CreationMethod)m).chain_up) {
								...
								foreach (TypeParameter type_param in current_class.get_type_parameters()) {
									...
									param_name = new CCodeIdentifier("%s_type".printf(type_param.name.ToLower()));
									assign = new CCodeAssignment(CCodeMemberAccess.pointer(priv_access, param_name.name), param_name);
									ccode.add_expression(assign);

									param_name = new CCodeIdentifier("%s_dup_func".printf(type_param.name.ToLower()));
									assign = new CCodeAssignment(CCodeMemberAccess.pointer(priv_access, param_name.name), param_name);
									ccode.add_expression(assign);

									param_name = new CCodeIdentifier("%s_destroy_func".printf(type_param.name.ToLower()));
									assign = new CCodeAssignment(CCodeMemberAccess.pointer(priv_access, param_name.name), param_name);
									ccode.add_expression(assign);
								}
							}
			...
			if (m.body != null) {
				...
				if (co_switch_block != null) {
					...
					for (int state = 1; state < emit_context.next_coroutine_state; state++) {
						ccode.add_case(new CCodeConstant(state.ToString()));
			...
			if (!m.is_abstract || (m.is_abstract && current_type_symbol is Class)) {
				...
				if (m.body != null) {
					...
					if (!(m.return_type is VoidType) && !m.return_type.is_real_non_null_struct_type() && !m.coroutine) {
						...
						if (m.return_block != null && m.return_block.get_predecessors().Count == 0) {
					...
					if (m is CreationMethod) {
						...
						if (current_type_symbol is Class && gobject_type != null && current_class.is_subtype_of(gobject_type)
							&& current_class.get_type_parameters().Count > 0
							...
							cunsetcall.add_argument(new CCodeUnaryExpression(CCodeUnaryOperator.ADDRESS_OF, CCodeMemberAccess.pointer(new CCodeIdentifier("__params_it"), "value")));
		...
		public virtual CCodeParameter generate_parameter(Parameter param, CCodeFile decl_space, Dictionary<int, CCodeParameter> cparam_map, Dictionary<int, CCodeExpression> carg_map) {
			...
			if (!param.ellipsis) {
				...
				cparam = CCodeParameter.with_ellipsis();
			}

			cparam_map[get_param_pos(get_ccode_pos(param), param.ellipsis)] = cparam;
			if (carg_map != null && !param.ellipsis) {
				carg_map[get_param_pos(get_ccode_pos(param), param.ellipsis)] = get_variable_cexpression(param.name);
			}
		...
		public override void generate_cparameters(Method m, CCodeFile decl_space, Dictionary<int, CCodeParameter> cparam_map, CCodeFunction func, CCodeFunctionDeclarator vdeclarator = null, Dictionary<int, CCodeExpression> carg_map = null, CCodeFunctionCall vcall = null, int direction = 3) {
			if (m.closure) {
				...
				cparam_map[get_param_pos(get_ccode_instance_pos(m))] = instance_param;
				...
				} else if (parent_type is ValaEnum) {
					this_type = new EnumValueType((ValaEnum)parent_type);
				} else {
					assert_not_reached();
				}
				...
				cparam_map[get_param_pos(get_ccode_instance_pos(m))] = class_param;
			}
			...
			if (is_gtypeinstance_creation_method(m)) {
				...
				foreach (TypeParameter type_param in cl.get_type_parameters()) {
					cparam_map[get_param_pos(0.1 * type_param_index + 0.01)] = new CCodeParameter("%s_type".printf(type_param.name.ToLower()), "GType");
					cparam_map[get_param_pos(0.1 * type_param_index + 0.02)] = new CCodeParameter("%s_dup_func".printf(type_param.name.ToLower()), "GBoxedCopyFunc");
					cparam_map[get_param_pos(0.1 * type_param_index + 0.03)] = new CCodeParameter("%s_destroy_func".printf(type_param.name.ToLower()), "GDestroyNotify");
					if (carg_map != null) {
						carg_map[get_param_pos(0.1 * type_param_index + 0.01)] = new CCodeIdentifier("%s_type".printf(type_param.name.ToLower()));
						carg_map[get_param_pos(0.1 * type_param_index + 0.02)] = new CCodeIdentifier("%s_dup_func".printf(type_param.name.ToLower()));
						carg_map[get_param_pos(0.1 * type_param_index + 0.03)] = new CCodeIdentifier("%s_destroy_func".printf(type_param.name.ToLower()));
					}
					type_param_index++;
				}
			...
			while (true) {
				...
				func.add_parameter(cparam_map[min_pos]);
				if (vdeclarator != null) {
		...
		public void generate_vfunc(Method m, DataType return_type, Dictionary<int, CCodeParameter> cparam_map, Dictionary<int, CCodeExpression> carg_map, string suffix = "", int direction = 3) {
			push_context(new EmitContext());

			string cname = get_ccode_name(m);
			if (suffix == "_finish" && cname.EndsWith("_async")) {
			...
			var vcall = new CCodeFunctionCall(CCodeMemberAccess.pointer(vcast, cname + suffix));
			carg_map[get_param_pos(get_ccode_instance_pos(m))] = new CCodeIdentifier("self");
			...
			} else if (m.get_postconditions().Count == 0) {
			...
			if (m.get_postconditions().Count > 0) {
		...
		private void create_aux_constructor(CreationMethod m, string func_name, bool self_as_first_parameter) {
			...
			if (m.is_variadic()) {
				...
				va_start.add_argument(carg_map[second_last_pos]);
