...
namespace CCodeGen.Modules
{
	public class GDBusClientModule : GDBusModule
	{
		...
		public CCodeConstant get_dbus_timeout(Symbol symbol) {
			...
			return new CCodeConstant(timeout.ToString());
		}
		...
		void generate_proxy_interface_init(Interface main_iface, Interface iface) {
			...
			foreach (Method m in iface.get_methods()) {
				...
				var vfunc_entry = CCodeMemberAccess.pointer(new CCodeIdentifier("iface"), get_ccode_vfunc_name(m));
				if (!m.coroutine) {
					...
					vfunc_entry = CCodeMemberAccess.pointer(new CCodeIdentifier("iface"), get_ccode_finish_vfunc_name(m));
			...
			foreach (Property prop in iface.get_properties()) {
				...
				if (prop.get_accessor != null) {
					var vfunc_entry = CCodeMemberAccess.pointer(new CCodeIdentifier("iface"), "get_" + prop.name);
					ccode.add_assignment(vfunc_entry, new CCodeIdentifier(generate_dbus_proxy_property_get(main_iface, iface, prop)));
				}
				if (prop.set_accessor != null) {
					var vfunc_entry = CCodeMemberAccess.pointer(new CCodeIdentifier("iface"), "set_" + prop.name);
		...
		public override void visit_interface(Interface iface) {
			...
			ccode.add_assignment(CCodeMemberAccess.pointer(proxy_class, "g_signal"), new CCodeIdentifier(lower_cname + "_g_signal"));
			pop_function();
		...
		public override void visit_method_call(MethodCall expr) {
			...
			var type_arg = ma.get_type_arguments()[0];
			...
			if (object_type != null) {
				...
				var _quark = new CCodeFunctionCall(new CCodeIdentifier("g_quark_from_static_string"));
				_quark.add_argument(new CCodeConstant("\"vala-dbus-proxy-type\""));

				var _get_qdata = new CCodeFunctionCall(new CCodeIdentifier("g_type_get_qdata"));
				_get_qdata.add_argument(get_type_id_expression(type_arg));
				_get_qdata.add_argument(_quark);

				proxy_type = new CCodeFunctionCall(new CCodeCastExpression(_get_qdata, "GType (*) (void)"));

				_quark = new CCodeFunctionCall(new CCodeIdentifier("g_quark_from_static_string"));
				_quark.add_argument(new CCodeConstant("\"vala-dbus-interface-name\""));

				_get_qdata = new CCodeFunctionCall(new CCodeIdentifier("g_type_get_qdata"));
				_get_qdata.add_argument(get_type_id_expression(type_arg));
				_get_qdata.add_argument(_quark);

				dbus_iface_name = _get_qdata;
			}
			...
			if (bus_get_proxy_async || conn_get_proxy_async) {
				if (ma.member_name == "end" && ma.inner.symbol_reference == ma.symbol_reference) {
					...
					var _args = expr.get_argument_list();
					Expression res = _args[0];
					...
					var _ccall = new CCodeFunctionCall(new CCodeIdentifier("g_async_initable_new_finish"));
					_ccall.add_argument(source_ref);
					_ccall.add_argument(get_cvalue(res));
					_ccall.add_argument(new CCodeUnaryExpression(CCodeUnaryOperator.ADDRESS_OF, get_variable_cexpression("_inner_error_")));

					var _temp_var = get_temp_variable(expr.value_type, expr.value_type.value_owned);
					var _temp_ref = get_variable_cexpression(_temp_var.name);
					emit_temp_var(_temp_var);
					ccode.add_assignment(_temp_ref, _ccall);
			...
			Expression name = args[base_arg_index + 0];
			Expression object_path = args[base_arg_index + 1];
			Expression flags = args[base_arg_index + 2];
			Expression cancellable = args[base_arg_index + 3];
			...
			if (bus_get_proxy_async || conn_get_proxy_async) {
				if (expr.is_yield_expression) {
					...
					// begin
					Expression callback = args[base_arg_index + 4];
			...
			if (bus_get_proxy_async || conn_get_proxy_async) {
				if (expr.is_yield_expression) {
					...
					ccall.add_argument(CCodeMemberAccess.pointer(new CCodeIdentifier("_data_"), "_source_object_"));
					// pass GAsyncResult stored in closure to finish function
					ccall.add_argument(CCodeMemberAccess.pointer(new CCodeIdentifier("_data_"), "_res_"));
					ccall.add_argument(new CCodeUnaryExpression(CCodeUnaryOperator.ADDRESS_OF, get_variable_cexpression("_inner_error_")));
				} else {
					// begin
					ccode.add_expression(ccall);
		...
		string generate_dbus_signal_handler(Signal sig, ObjectTypeSymbol sym) {
			...
			foreach (Parameter param in sig.get_parameters()) {
				...
				ccode.add_declaration(get_ccode_name(owned_type), CCodeVariableDeclarator.zero(param_name, default_value_for_type(param.variable_type, true)));
		...
		void generate_marshalling(Method m, CallType call_type, string iface_name, string method_name, int method_timeout) {
			...
			bool has_error_argument = (m.get_error_types().Count > 0);
			...
			if (call_type == CallType.SYNC || call_type == CallType.FINISH) {
				...
				if (uses_fd) {
					ccode.add_declaration("gint", CCodeVariableDeclarator.zero("_fd_index", new CCodeConstant("0")));
				...
				if (has_result) {
					...
					foreach (Parameter param in m.get_parameters()) {
						if (param.direction == ParameterDirection.OUT) {
							ccode.add_declaration(get_ccode_name(param.variable_type), CCodeVariableDeclarator.zero("_vala_%s".printf(param.name), default_value_for_type(param.variable_type, true)));
							...
							receive_dbus_value(param.variable_type, new CCodeIdentifier("_reply_message"), new CCodeIdentifier("_reply_iter"), target, param, out may_fail, error_argument);
					...
					if (!(m.return_type is VoidType)) {
						if (m.return_type.is_real_non_null_struct_type()) {
							...
							bool may_fail;
							receive_dbus_value(m.return_type, new CCodeIdentifier("_reply_message"), new CCodeIdentifier("_reply_iter"), target, m, out may_fail);
						} else {
							ccode.add_declaration(get_ccode_name(m.return_type), CCodeVariableDeclarator.zero("_result", default_value_for_type(m.return_type, true)));
							...
							bool may_fail;
							receive_dbus_value(m.return_type, new CCodeIdentifier("_reply_message"), new CCodeIdentifier("_reply_iter"), new CCodeIdentifier("_result"), m, out may_fail, new CCodeIdentifier("error"));
		...
		string generate_dbus_proxy_property_get(Interface main_iface, Interface iface, Property prop) {
			...
			if (prop.property_type.is_real_non_null_struct_type()) {
				var target = new CCodeUnaryExpression(CCodeUnaryOperator.POINTER_INDIRECTION, new CCodeIdentifier("result"));
				bool may_fail;
				var result = deserialize_expression(prop.get_accessor.value_type, new CCodeIdentifier("_inner_reply"), target, out may_fail);
				...
				if (get_dbus_signature(prop) != null) {
					...
					bool may_fail;
					var result = deserialize_expression(prop.get_accessor.value_type, new CCodeIdentifier("_inner_reply"), new CCodeIdentifier("_result"), out may_fail);
