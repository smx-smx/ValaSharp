	...
	public class GDBusServerModule : GDBusClientModule
	{
		string generate_dbus_wrapper(Method m, ObjectTypeSymbol sym, bool ready = false) {
			...
			if (!m.coroutine || ready) {
				ccode.add_declaration("GError*", CCodeVariableDeclarator.zero("error", new CCodeConstant("NULL")));
				uses_error = true;
			}
				...
				foreach (Parameter param in m.get_parameters()) {
					...
					if (array_type != null) {
						for (int dim = 1; dim <= array_type.rank; dim++) {
							...
							ccode.add_declaration("int", CCodeVariableDeclarator.zero(length_cname, new CCodeConstant("0")));
						}
					}
					...
					bool may_fail;
					receive_dbus_value(param.variable_type, message_expr, new CCodeIdentifier("_arguments_iter"), new CCodeIdentifier(param_name), param, out may_fail, new CCodeUnaryExpression(CCodeUnaryOperator.ADDRESS_OF, new CCodeIdentifier("error")));
			...
			if (!no_reply && (!m.coroutine || ready)) {
				...
				ccode.add_declaration("GDBusMessage*", CCodeVariableDeclarator.zero("_reply_message", new CCodeConstant("NULL")));
				...
				if (!(m.return_type is VoidType)) {
					if (m.return_type.is_real_non_null_struct_type()) {
						ccode.add_declaration(get_ccode_name(m.return_type), CCodeVariableDeclarator.zero("result", default_value_for_type(m.return_type, true)));
		...
		string generate_dbus_signal_wrapper(Signal sig, ObjectTypeSymbol sym, string dbus_iface_name) {
			...
			foreach (var param in sig.get_parameters()) {
				...
				var cparam = generate_parameter(param, cfile, new Dictionary<int, CCodeParameter>(), null);
		...
		string generate_dbus_property_get_wrapper(Property prop, ObjectTypeSymbol sym) {
			...
			if (prop.get_accessor.value_type.is_real_non_null_struct_type()) {
				ccode.add_declaration(get_ccode_name(prop.get_accessor.value_type), CCodeVariableDeclarator.zero("result", default_value_for_type(prop.get_accessor.value_type, true)));
				ccall.add_argument(new CCodeUnaryExpression(CCodeUnaryOperator.ADDRESS_OF, new CCodeIdentifier("result")));
		...
		string generate_dbus_property_set_wrapper(Property prop, ObjectTypeSymbol sym) {
			...
			ccode.add_declaration(get_ccode_name(owned_type), CCodeVariableDeclarator.zero("value", default_value_for_type(prop.property_type, true)));
		...
		void generate_interface_method_call_function(ObjectTypeSymbol sym) {
			...
			foreach (Method m in sym.get_methods()) {
				...
				ccall = new CCodeFunctionCall(new CCodeIdentifier(generate_dbus_wrapper(m, sym)));
