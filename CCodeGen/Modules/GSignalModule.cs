	...
	public class GSignalModule : GObjectModule
	{
		...
		private string get_value_type_name_from_type_reference(DataType t) {
			if (t is PointerType || t.type_parameter != null) {
				...
				return "gpointer";
			} else if (t is ValaValueType && t.nullable) {
				return "gpointer";
			...
			} else if (t.data_type is ValaEnum) {
				return "gint";
		...
		private string get_marshaller_signature(List<Parameter> _params, DataType return_type) {
			string signature;
		...
		public override void visit_signal(Signal sig) {
			...
			foreach (Parameter p in sig.get_parameters()) {
				generate_parameter(p, cfile, new Dictionary<int, CCodeParameter>(), null);
			}
		...
		public override void visit_method_call(MethodCall expr) {
			...
			var handler = expr.get_argument_list()[0];
		...
		CCodeExpression connect_signal(Signal sig, Expression signal_access, Expression handler, bool disconnect, bool after, CodeNode expr) {
			...
			if (signal_access is ElementAccess) {
				...
				var detail_expr = ea.get_indices()[0];
