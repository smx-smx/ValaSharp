...
namespace CCodeGen.Modules {
	public class GTypeModule : GErrorModule {
		public override CCodeParameter generate_parameter(Parameter param, CCodeFile decl_space, Dictionary<int, CCodeParameter> cparam_map, Dictionary<int, CCodeExpression> carg_map) {
			...
			cparam_map[get_param_pos(get_ccode_pos(param))] = cparam;
			if (carg_map != null) {
		...
		public override void generate_class_struct_declaration(Class cl, CCodeFile decl_space) {
			...
			if (cl.is_compact && cl.base_class == null && cl.get_fields().Count == 0) {
		...
		void generate_class_private_declaration(Class cl, CCodeFile decl_space) {
			...
			if (is_gtypeinstance) {
				...
				if (cl.has_private_fields || cl.get_type_parameters().Count > 0) {
		...
		public override void visit_class(Class cl) {
			...
			if (is_gtypeinstance) {
				...
				prop_array_decl.add_declarator(new CCodeVariableDeclarator("%s_properties".printf(get_ccode_lower_case_name(cl)), null, CCodeDeclaratorSuffix.with_array(new CCodeIdentifier(last_prop))));
			...
			if (is_gtypeinstance) {
				if (is_fundamental) {
					...
					var ref_count = new CCodeAssignment(CCodeMemberAccess.pointer(new CCodeIdentifier("self"), "ref_count"), new CCodeConstant("1"));
				...
				if (cl.get_signals().Count > 0) {
					...
					signal_array_decl.add_declarator(new CCodeVariableDeclarator("%s_signals".printf(get_ccode_lower_case_name(cl)), new CCodeConstant("{0}"), CCodeDeclaratorSuffix.with_array(new CCodeIdentifier(last_signal))));
				...
				if (!cl.is_compact && (cl.get_fields().Count > 0 || cl.destructor != null || cl.is_fundamental())) {
				...
				if (is_fundamental) {
					var ref_count = CCodeMemberAccess.pointer(new CCodeIdentifier("self"), "ref_count");
					...
					ccall = new CCodeFunctionCall(CCodeMemberAccess.pointer(ccast, "finalize"));
		...
		private void add_type_value_table_init_function(Class cl) {
			...
			ccode.add_assignment(new CCodeMemberAccess(CCodeMemberAccess.pointer(new CCodeIdentifier("value"), "data[0]"), "v_pointer"), new CCodeConstant("NULL"));
			pop_function();
		...
		private void add_type_value_table_free_function(Class cl) {
			...
			var vpointer = new CCodeMemberAccess(CCodeMemberAccess.pointer(new CCodeIdentifier("value"), "data[0]"), "v_pointer");
		...
		private void add_type_value_table_copy_function(Class cl) {
			...
			var dest_vpointer = new CCodeMemberAccess(CCodeMemberAccess.pointer(new CCodeIdentifier("dest_value"), "data[0]"), "v_pointer");
			var src_vpointer = new CCodeMemberAccess(CCodeMemberAccess.pointer(new CCodeIdentifier("src_value"), "data[0]"), "v_pointer");
		...
		private void add_type_value_table_collect_value_function(Class cl) {
			...
			var l_expression = new CCodeMemberAccess(CCodeMemberAccess.pointer(obj_identifier, "parent_instance"), "g_class");
		...
		private void add_g_param_spec_type_function(Class cl) {
			...
			ccode.add_assignment(CCodeMemberAccess.pointer(ccall, "value_type"), new CCodeIdentifier("object_type"));
		...
		private void begin_class_init_function(Class cl) {
			...
			if (!cl.is_compact && !cl.is_subtype_of(gobject_type) && (cl.get_fields().Count > 0 || cl.destructor != null || cl.is_fundamental())) {
				...
				var finalize_assignment = new CCodeAssignment(CCodeMemberAccess.pointer(ccast, "finalize"), new CCodeIdentifier("%sfinalize".printf(get_ccode_lower_case_prefix(cl))));
			...
			foreach (Method m in cl.get_methods()) {
				...
				if (!m.is_abstract || !m.coroutine) {
					...
					ccode.add_assignment(CCodeMemberAccess.pointer(ccast, get_ccode_vfunc_name(m.base_method)), cfunc);

					if (m.coroutine) {
						cfunc = new CCodeIdentifier(get_ccode_finish_real_name(m));
						cfunc = cast_method_pointer(m.base_method, cfunc, base_type, 2);
						ccode.add_assignment(CCodeMemberAccess.pointer(ccast, get_ccode_finish_vfunc_name(m.base_method)), cfunc);
					}
				}
			}
			...
			foreach (Signal sig in cl.get_signals()) {
				...
				ccode.add_assignment(CCodeMemberAccess.pointer(ccast, get_ccode_vfunc_name(sig.default_handler)), new CCodeIdentifier(get_ccode_real_name(sig.default_handler)));
			}
		...
		private void add_generic_accessor_function(string base_name, string return_type, CCodeExpression expression, TypeParameter p, Class cl, Interface iface) {
			...
			ccode.add_assignment(CCodeMemberAccess.pointer(ciface, base_name), cfunc);
		}
		...
		private void add_interface_init_function(Class cl, Interface iface) {
			...
			foreach (Method m in cl.get_methods()) {
				...
				ccode.add_assignment(CCodeMemberAccess.pointer(ciface, get_ccode_vfunc_name(m.base_interface_method)), cfunc);
				...
				if (m.coroutine) {
					...
					ccode.add_assignment(CCodeMemberAccess.pointer(ciface, get_ccode_finish_vfunc_name(m.base_interface_method)), cfunc);
				}
			}
			...
			if (iface.get_attribute("GenericAccessors") != null) {
				foreach (TypeParameter p in iface.get_type_parameters()) {
					...
					add_generic_accessor_function("get_%s_type".printf(p.name.ToLower()),
												   "GType",
												   get_type_id_expression(p_data_type),
												   p, cl, iface);

					add_generic_accessor_function("get_%s_dup_func".printf(p.name.ToLower()),
												   "GBoxedCopyFunc",
												   get_dup_func_expression(p_data_type, null, false),
												   p, cl, iface);

					add_generic_accessor_function("get_%s_destroy_func".printf(p.name.ToLower()),
												   "GDestroyNotify",
			...
			foreach (Method m in iface.get_methods()) {
				if (m.is_abstract) {
					...
					if (base_class != null && cl_method.parent_symbol != cl) {
						...
						var ciface = new CCodeIdentifier("iface");
						ccode.add_assignment(CCodeMemberAccess.pointer(ciface, get_ccode_vfunc_name(m)), cfunc);
					}
				}
			}
			...
			foreach (Property prop in cl.get_properties()) {
				...
				if (!get_ccode_no_accessor_method(prop.base_interface_property) && !get_ccode_concrete_accessor(prop.base_interface_property)) {
					...
					if (prop.set_accessor != null) {
						...
						ccode.add_assignment(CCodeMemberAccess.pointer(ciface, "set_%s".printf(prop.name)), cfunc);
					}
				}
			}
		...
		private void begin_instance_init_function(Class cl) {
			...
			if (!cl.is_compact && (cl.has_private_fields || cl.get_type_parameters().Count > 0)) {
				var ccall = new CCodeFunctionCall(new CCodeIdentifier("%s_GET_PRIVATE".printf(get_ccode_upper_case_name(cl, null))));
				ccall.add_argument(new CCodeIdentifier("self"));
				func.add_assignment(CCodeMemberAccess.pointer(new CCodeIdentifier("self"), "priv"), ccall);
			}
		...
		private void add_finalize_function(Class cl) {
			if (!cl.is_compact) {
				...
				if (cl.base_class != null) {
					...
					var ccall = new CCodeFunctionCall(CCodeMemberAccess.pointer(ccast, "finalize"));
		...
		public override CCodeExpression get_param_spec(Property prop) {
			...
			} else if (prop.property_type.data_type is ValaEnum) {
				var e = prop.property_type.data_type as ValaEnum;
		...
		public override void generate_interface_declaration(Interface iface, CCodeFile decl_space) {
			...
			if (iface.get_attribute("GenericAccessors") != null) {
				foreach (TypeParameter p in iface.get_type_parameters()) {
					string method_name = "get_%s_type".printf(p.name.ToLower());
					...
					method_name = "get_%s_dup_func".printf(p.name.ToLower());
					...
					method_name = "get_%s_destroy_func".printf(p.name.ToLower());
					vdeclarator = new CCodeFunctionDeclarator(method_name);
					this_type = get_data_type_for_symbol(iface);
					vdeclarator.add_parameter(new CCodeParameter("self", get_ccode_name(this_type)));
		...
		public override void visit_interface(Interface iface) {
			...
			if (iface.get_signals().Count > 0) {
				...
				var signal_array_decl = new CCodeDeclaration("guint");
				signal_array_decl.modifiers |= CCodeModifiers.STATIC;
				signal_array_decl.add_declarator(new CCodeVariableDeclarator("%s_signals".printf(get_ccode_lower_case_name(iface)), new CCodeConstant("{0}"), CCodeDeclaratorSuffix.with_array(new CCodeIdentifier(last_signal))));
				cfile.add_type_declaration(signal_array_decl);
			}
		...
		private void add_interface_base_init_function(Interface iface) {
			...
			foreach (Signal sig in iface.get_signals()) {
				...
				ccode.add_assignment(CCodeMemberAccess.pointer(ciface, get_ccode_vfunc_name(sig.default_handler)), new CCodeIdentifier(cname));
			}
			...
			foreach (Method m in iface.get_methods()) {
				if (m.is_virtual) {
					...
					ccode.add_assignment(CCodeMemberAccess.pointer(ciface, get_ccode_vfunc_name(m)), new CCodeIdentifier(cname));
					if (m.coroutine) {
						ccode.add_assignment(CCodeMemberAccess.pointer(ciface, get_ccode_finish_vfunc_name(m)), new CCodeIdentifier(get_ccode_finish_real_name(m)));
					}
				}
			}
		...
		public override void visit_method_call(MethodCall expr) {
			...
			set_cvalue(expr, new CCodeConditionalExpression(is_null_value, CCodeMemberAccess.pointer(get_variable_cexpression(temp_var.name), "value_name"), new CCodeIdentifier("NULL")));
			pop_line();
		}
