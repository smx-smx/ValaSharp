	...
	public class CCodeAttribute : AttributeCache {
		...
		public bool ref_function_void {
			get {
				...
				return _ref_function_void.Value;
			}
		}
		...
		public bool free_function_address_of {
			get {
				...
				return _free_function_address_of.Value;
			}
		}
			...
			get {
				if (_pos == null) {
					if (ccode != null && ccode.has_argument("pos")) {
						...
						if (sym is Callable) {
							_pos = ((Callable)sym).get_parameters().IndexOf(param) + 1.0;
						} else {
							_pos = 0.0;
						}
					}
		...
		public bool array_null_terminated {
			get {
				...
				return _array_null_terminated.Value;
			}
		}
		...
		private string get_default_lower_case_suffix() {
			if (sym is ObjectTypeSymbol) {
				...
				if (csuffix.StartsWith("type_")) {
					csuffix = "type" + csuffix.Substring("type_".Length);
				} else if (csuffix.StartsWith("is_")) {
					csuffix = "is" + csuffix.Substring("is_".Length);
				}
				if (csuffix.EndsWith("_class")) {
		...
		private string get_default_marshaller_type_name() {
			...
			} else if (node is ValaValueType && ((ValaValueType)node).nullable) {
				return "POINTER";
