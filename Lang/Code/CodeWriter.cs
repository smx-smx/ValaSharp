	...
	public class CodeWriter : CodeVisitor {
		...
		public void write_file(CodeContext context, string filename) {
			...
			var header = context.version_header ?
				"/* %s generated by %s %s, do not modify. */".printf(Path.GetFileName(filename), AppDomain.CurrentDomain.FriendlyName, Config.BUILD_VERSION) :
				"/* %s generated by %s, do not modify. */".printf(Path.GetFileName(filename), AppDomain.CurrentDomain.FriendlyName);
			write_string(header);
			write_newline();
			write_newline();
		...
		public override void visit_using_directive(UsingDirective ns) {
			if (type == CodeWriterType.FAST) {
				...
				var symbols = new List<UnresolvedSymbol>();
				...
				write_string(symbols[0].name);

				for (int i = 1; i < symbols.Count; i++) {
					write_string(".");
					write_string(symbols[i].name);
				}
		...
		public override void visit_namespace(Namespace ns) {
			...
			if (context.vapi_comments && comments.Count > 0) {
				bool first = true;
			...
			visit_sorted(ns.get_namespaces().ConvertAll(x => (Symbol)x));
			visit_sorted(ns.get_classes().ConvertAll(x => (Symbol)x));
			visit_sorted(ns.get_interfaces().ConvertAll(x => (Symbol)x));
			visit_sorted(ns.get_structs().ConvertAll(x => (Symbol)x));
			visit_sorted(ns.get_enums().ConvertAll(x => (Symbol)x));
			visit_sorted(ns.get_error_domains().ConvertAll(x => (Symbol)x));
			visit_sorted(ns.get_delegates().ConvertAll(x => (Symbol)x));
			visit_sorted(ns.get_fields().ConvertAll(x => (Symbol)x));
			visit_sorted(ns.get_constants().ConvertAll(x => (Symbol)x));
			visit_sorted(ns.get_methods().ConvertAll(x => (Symbol)x));
		...
		public override void visit_class(Class cl) {
			...
			visit_sorted(cl.get_classes().ConvertAll(x => (Symbol)x));
			visit_sorted(cl.get_structs().ConvertAll(x => (Symbol)x));
			visit_sorted(cl.get_enums().ConvertAll(x => (Symbol)x));
			visit_sorted(cl.get_delegates().ConvertAll(x => (Symbol)x));
			visit_sorted(cl.get_fields().ConvertAll(x => (Symbol)x));
			visit_sorted(cl.get_constants().ConvertAll(x => (Symbol)x));
			visit_sorted(cl.get_methods().ConvertAll(x => (Symbol)x));
			visit_sorted(cl.get_properties().ConvertAll(x => (Symbol)x));
			visit_sorted(cl.get_signals().ConvertAll(x => (Symbol)x));
		...
		void visit_sorted(IList<Symbol> symbols) {
			...
			var sorted_symbols = new List<Symbol>();
			foreach (Symbol sym in symbols) {
				int left = 0;
				int right = sorted_symbols.Count - 1;
				if (left > right || sym.name.CompareTo(sorted_symbols[left].name) < 0) {
					sorted_symbols.Insert(0, sym);
				} else if (sym.name.CompareTo(sorted_symbols[right].name) > 0) {
					sorted_symbols.Add(sym);
				} else {
					while (right - left > 1) {
						int i = (right + left) / 2;
						if (sym.name.CompareTo(sorted_symbols[i].name) > 0) {
							left = i;
						} else {
							right = i;
						}
		...
		public override void visit_interface(Interface iface) {
			...
			visit_sorted(iface.get_classes().ConvertAll(x => (Symbol)x));
			visit_sorted(iface.get_structs().ConvertAll(x => (Symbol)x));
			visit_sorted(iface.get_enums().ConvertAll(x => (Symbol)x));
			visit_sorted(iface.get_delegates().ConvertAll(x => (Symbol)x));
			visit_sorted(iface.get_fields().ConvertAll(x => (Symbol)x));
			visit_sorted(iface.get_constants().ConvertAll(x => (Symbol)x));
			visit_sorted(iface.get_methods().ConvertAll(x => (Symbol)x));
			visit_sorted(iface.get_properties().ConvertAll(x => (Symbol)x));
			visit_sorted(iface.get_signals().ConvertAll(x => (Symbol)x));
		...
		public override void visit_enum(ValaEnum en) {
			...
			if (!first) {
				if (en.get_methods().Count > 0 || en.get_constants().Count > 0) {
					write_string(";");
				}
				write_newline();
			}
		...
		public override void visit_error_domain(ErrorDomain edomain) {
			...
			if (!first) {
				if (edomain.get_methods().Count > 0) {
					write_string(";");
				}
				write_newline();
			}
		...
		private void write_params(List<Parameter> _params) {
			write_string("(");
		...
		public override void visit_boolean_literal(BooleanLiteral lit) {
			write_string(lit.value.ToString());
		}
		...
		public override void visit_lambda_expression(LambdaExpression expr) {
			write_string("(");
			var _params = expr.get_parameters();
			int i = 1;
		...
		private void write_attributes(CodeNode node) {
			...
			var attributes = new SortedSet<ValaAttribute>();
			foreach (var attr in node.attributes) {
				attributes.Add(attr);
			}
			if (need_cheaders && node.get_attribute("CCode") == null) {
				attributes.Add(new ValaAttribute("CCode"));
			}
			...
			while (iter.MoveNext()) {
				...
				if (need_cheaders && attr.name == "CCode" && !attr.has_argument("cheader_filename")) {
					keys.Add("cheader_filename");
				}

				if (attr.name == "CCode" && keys.Count == 0) {
					// only cheader_filename on namespace
					continue;
				}

				if (sym != null && attr.args.Count == 1 && attr.name == "Version") {
				...
				if (keys.Count > 0) {
					...
					while (arg_iter.MoveNext()) {
						...
						if (arg_name == "cheader_filename") {
							...
							stream.printf("%s%s = %s", separator, arg_name, attr.args[arg_name]);
						}
						separator = ", ";
					}
