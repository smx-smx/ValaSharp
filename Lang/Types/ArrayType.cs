...
namespace Vala.Lang.Types
{
	 ...
	 */
		...
		private ArrayLengthField get_length_field() {
			if (length_field == null) {
				...
				if (rank > 1) {
					...
					ValaValueType integer = new IntegerType((Struct)root_symbol.scope.lookup("int"));
		...
		public override string to_qualified_string(Scope scope) {
			...
			if (!fixed_length) {
				return "%s[%s]%s".printf(elem_str, gstring.nfill(rank - 1, ','), nullable ? "?" : "");
			} else {
				return elem_str;
			}
		}
		...
		public override bool compatible(DataType target_type) {
			...
			if (element_type is ValaValueType && element_type.nullable != target_array_type.element_type.nullable) {
				return false;
			}
